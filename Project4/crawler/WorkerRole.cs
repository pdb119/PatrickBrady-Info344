...
namespace crawler
{
    public class WorkerRole : RoleEntryPoint
    {
        ...
        private object numberCrawledLock = new Object();
        //public 
        ...
        public override void Run()
        {
            ...
            //allow thread count to be changed without republishing (from remote desktop
            Thread[] threads = new Thread[int.Parse(ConfigurationManager.AppSettings["numCores"])];
            ...
            while (true)
            {
                ...
                {                    
                    ...
                    {
                        threads[i] = new Thread(new ThreadStart(processThread));                        
                        threads[i].Start();
            ...
            while (true)
            {
                ...
                if (qmessage == null)
                {
                    ...
                    if (!alreadyProcessed)
                    {
                        ...
                        foreach (string title in page.title.Split(' '))
                        {
                            TableOperation insertPages = TableOperation.InsertOrReplace(new SiteIndex(url.Host, url.AbsolutePath, title));
                            siteTable.Execute(insertPages);
                        }                                                                                      
                        lock (numberCrawledLock)
                        {
                            numberCrawled++;
                        }
